...
CROP_FLOOR = ((170, 0), (rc.camera.get_height(), rc.camera.get_width()))
...
#returns the turn angle based on the error of the center of contour and set point, *kP
def mapping(angle):
    angle = kP * ((contour_center[1] * 2 / rc.camera.get_width()) -1)
    if angle > 1:
        angle = 1
    elif angle < -1:
        angle = -1
